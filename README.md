...
- **Raft 共识**：内建 Raft 强一致复制，支持 KV / Hash / List / Set 写操作多节点同步
- **可观测性**：结构化日志（JSON/Text）+ Prometheus 指标 /metrics 端点 + 进程资源监控
...
| 命令 | 说明 |
|------|------|
| `set <k> <v> [--ttl 30s]` | 写入键值，TTL 支持 s/m/h |
| `get <k>` | 读取键值 |
| `del <k>` | 删除键 |
| `len` | 总条目数 |
| `cleanup` | 清理过期条目 |
...
启用后，所有写操作（KV / Hash / List / Set）自动通过 Raft 日志复制到多数节点。读操作仍直接查询本地缓存。非 Leader 节点收到写请求会返回 `not leader`，客户端需重定向到 Leader。

参见 [集群管理](docs/cluster.md) 和 [Raft 共识](docs/raft.md)。
...
实时输出进程级资源：堆内存使用（含上限进度条）、goroutine 数、缓存条目数、连接数、网络 IO 吞吐量。

// Package main benchmarks Redis with configurable KV sizes and operation modes.
//
// Usage:
//
//	go run ./bench_redis --addr 127.0.0.1:6379 --mode set --key-size 64 --val-size 256 --ops 100000 --goroutines 64
//	go run ./bench_redis --addr 127.0.0.1:6379 --mode get  --key-size 64 --val-size 1024 --ops 100000
//	go run ./bench_redis --addr 127.0.0.1:6379 --mode mix --key-size 256 --val-size 4096 --ops 200000 --goroutines 128
//
...
func main() {
	...
	pipelineSize := flag.Int("pipeline", 0, "pipeline batch size (0=disabled)")
	...
	pipLabel := "off"
	if *pipelineSize > 0 {
		pipLabel = fmt.Sprintf("%d", *pipelineSize)
	}
	title := fmt.Sprintf("Redis OPS Benchmark — %s | key=%dB val=%dB (%s) | %d goroutines",
	...
	fmt.Printf("  ops:      %d  | goroutines: %d  | pool: %d  | keyspace: %d  | pipeline: %s\n\n",
		*totalOps, *goroutines, ps, nKeys, pipLabel)
...
func newClient(cfg benchConfig) *redis.Client {
